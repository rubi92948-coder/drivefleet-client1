...
const Navbar = () => {
  ...
  return (
    <nav className="bg-black text-white border-b border-gray-800 sticky top-0 z-50">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-gray-900 border border-gray-700 rounded-lg shadow-2xl z-[100] overflow-hidden">
                  <Link href="/add-car" onClick={() => setDropdownOpen(false)} className="block px-4 py-2 hover:bg-gray-800">Add Car</Link>
                  <Link href="/bookings" onClick={() => setDropdownOpen(false)} className="block px-4 py-2 hover:bg-gray-800">My Bookings</Link>
                  <Link href="/my-added-cars" onClick={() => setDropdownOpen(false)} className="block px-4 py-2 hover:bg-gray-800">My Added Cars</Link>
      ...
      {/* MOBILE MENU */}
      {open && (
        <div className="md:hidden px-4 pb-4 space-y-2 bg-gray-950">
          <Link href="/" className="block px-4 py-2" onClick={() => setOpen(false)}>Home</Link>
          <Link href="/explore-cars" className="block px-4 py-2" onClick={() => setOpen(false)}>Explore</Link>
          <Link href="/add-car" className="block px-4 py-2" onClick={() => setOpen(false)}>Add Car</Link>
          <Link href="/bookings" className="block px-4 py-2" onClick={() => setOpen(false)}>Bookings</Link>
        </div>
      )}
    </nav>
  );
